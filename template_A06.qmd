...
[https://github.com/jeanniexwu/Wu_Jeannie_201_A06](https://github.com/jeanniexwu/Wu_Jeannie_201_A06)
...
# using naming to add "height" for each player
...
print(player_info)
# player_info <- append(player_info$MJ, ''height' = 78')
...
# Use for loop for each player
for (i in names(player_info)){
  # access each player's list first
  player <- player_info[[i]]
  # then find the player's points and games played
  player_ppg <- player$points/player$games_played
  # add each player's points per game to the actual list
  points_per_game[[i]] <- player_ppg
}
...
```{r}
# create a rounding function to use later
...
# apply previous rounding function to each value
apply(mat_rand, 1:2, round_function)
```
...
```{r}
# use [, 3:14] to apply to every row from column 3 to 14
mean_per_month <- apply(max_temps[, 3:14], 2, mean)
...
```{r}
# index state from max_temps
CO_temp <- max_temps[max_temps$state == "CO",]
OH_temp <- max_temps[max_temps$state == "OH",]
VA_temp <- max_temps[max_temps$state == "VA",]
...
max_temp <- max(median_annual_temp)
min_temp <- min(median_annual_temp)

# use which.max to find the year, then index it from CO_temp
max_year <- CO_temp$year[which.max(median_annual_temp)]
min_year <- CO_temp$year[which.min(median_annual_temp)]

sprintf("The maximum median annual temperature of %.2f occurred in %d and the minimum median annual temperature of %.2f occurred in %d.", max_temp, max_year, min_temp, min_year)
...
sum_digits <- function(number){
  # checks if number is 0, returning 0 if so
  if(number == 0){
    return(0)
  }
  # accounts for the last digit
  last_digit = number %% 10
  remaining_digit = number %/% 10
  # recursive function continues to run through remaining digits
  return(last_digit + sum_digits(remaining_digit))
}
